		...
		if (typeCode.equals("H")) {
			...
			for (int i = 0 ; i < linList.size(); i++) {
                ...
                String where = "RI_STATUS = '예매' AND RI_LINE_ID = '" + ri.getFr_code() + ri.getTo_code() + "'"
    					+ " AND RI_FRDATE = '" + ri.getRi_frdate() + " " + fr_time.substring(0, 2) + ":" + fr_time.substring(2) + ":00'";
                ...
                si.setRi_com(ri_com);
                si.setLevel(level);
                si.setAdult_pee(adult_fee);
                si.setStudent_pee(student_fee);
                si.setChild_pee(child_fee);
			...
			for (int i = 0 ; i < linList.size(); i++) {
	            ...
	            if (convertToMinutes(fr_time) > convertToMinutes(formattedNowTime)) {
	            	...
	            	String where = "RI_STATUS = '예매' AND RI_LINE_ID = '" + route_id + "'"
	            			+ " AND RI_FRDATE = '" + ri.getRi_frdate() + " " + fr_time.substring(0, 2) + ":" + fr_time.substring(2) + ":00'";

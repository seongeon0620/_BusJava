...
public class SalesCtrl {
	...
	public String paymoneyList(Model model, HttpServletRequest request) throws Exception {
		...
		String ctgrTop = salesSvc.getTopCtgr();
		List<String> salseOld = salesSvc.getSales(1);
		List<String> salse = salesSvc.getSales(0);

		model.addAttribute("paymoneyList", paymoneyList);
		model.addAttribute("salseOld", salseOld);
		model.addAttribute("salse", salse);
		model.addAttribute("ctgrTop", ctgrTop);

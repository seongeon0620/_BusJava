...
%>
			...
			<p class="text-gray">중분류</p>
				<div class="col-12 text-center nav flex-column nav-pills p-0" id="middle">
				</div>
			</div>
			<div class="col-4 text-center p-0">
			<p class="text-gray">소분류</p>
				<div class="col-12 text-center nav flex-column nav-pills p-0" id="small">
				</div>
			</div>
		</div>
	</div>
        ...
        success: function(data) {
        	if (data.length > 0) {
                ...
                data.forEach(function(list) {
                	if (kind == "big") {
                		btnHTML += '<button type="button" class="over nav-link" onclick="getCtgr(\'' + list.code 
                				+ '\', \'middle\', \''+ list.name + '\');" value="' + list.code + '" >'+ list.name + '</button>';	
                	} else {
                		btnHTML += '<button type="button" class="over nav-link" value="' + list.code + '" onclick="changeTxt(\''+ 
                				list.name + '\', \''+ list.code + '\');">'+ list.name + '</button>';
                	}
                });

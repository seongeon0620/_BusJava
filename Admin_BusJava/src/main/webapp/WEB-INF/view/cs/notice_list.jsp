					...
					<div class="card-body">
						<form name="frmSch">
							...
							<div class="d-flex justify-content-center">
								...
								<button type="submit" class="btn waves-effect waves-light btn-secondary mb-2 ml-2" id="schBtn">
									...
									<tr>
										<th class="align-middle">
											<div class="custom-control custom-checkbox">
												<input type="checkbox" class="custom-control-input" id="checkall" name="checkall" onclick="chkAll(this);"> <label class="custom-control-label" for="checkall"></label>
											</div>
										</th>
										<th>No</th>
										<th>제목</th>
										...
										<c:forEach items="${aNoticeList }" var="anl" varStatus="status">
											<tr class="tr">
												<td class="align-middle">
													<div class="custom-control custom-checkbox">
														<input type="checkbox" class="custom-control-input" id="customCheck${anl.getNl_idx() }" name="chk" value="${anl.getNl_idx() }" onclick="chkOne(this);"> <label class="custom-control-label" for="customCheck${anl.getNl_idx() }"></label>
													</div>
												</td>
												<td>
													<span class="badge badge-pill badge-primary">중요</span>
												</td>
												<td class="text-left">
													<a href="noticeView?nlidx=${anl.getNl_idx() }${pi.getArgs() }">${anl.getNl_title() }</a>
												</td>
									...
									<c:if test="${noticeList.size() > 0 }">
										<c:forEach items="${noticeList }" var="nl" varStatus="status">
											...
											<tr class="tr">
												<td class="align-middle">
													<div class="custom-control custom-checkbox">
														<input type="checkbox" class="custom-control-input" id="customCheck${nl.getNl_idx() }" name="chk" value="${nl.getNl_idx() }" onclick="chkOne(this);"> <label class="custom-control-label" for="customCheck${nl.getNl_idx() }"></label>
													</div>
												</td>
												<td>${pi.getNum() - status.index}</td>
												<td class="text-left">
													<a href="noticeView?nlidx=${nl.getNl_idx() }${pi.getArgs()} ">${nl.getNl_title() }</a>
												</td>
												<td>${nl.getNl_date() }</td>

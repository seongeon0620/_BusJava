...
function chkChange(isview){
	...
	if (flidx == "")	alert("변경할 게시글을 선택하세요.");
	else				statusChange(flidx, isview);
}
...
<div class="container-fluid">
	...
	<tbody>
		...
		<c:set var="count" value="${count + 1}" />
		    <div class="form-check form-check-inline">
		        <div class="custom-control custom-checkbox">
		         	<input type="checkbox" name="all" value="all" class="custom-control-input" id="all" onclick="ctgrAll(this);" 
		         		<c:if test="${allChk == 'all'}">checked="checked"</c:if> />
		...
		<c:forEach items="${ctgrList}" var="cl" varStatus="status">
		    <div class="form-check form-check-inline">
		        <div class="custom-control custom-checkbox">
		            <input type="checkbox" name="schctgr" value="${cl.getFl_ctgr()}" class="custom-control-input" id="${status.count}" onclick="ctgrOne(this);" 
		            <c:choose>
		            ...
		            />
		            <label class="custom-control-label" for="${status.count}">${cl.getFl_ctgr()}</label>
		        </div>
		    </div>
		</c:forEach>
		</c:if>
			...
			<c:forEach items="${faqList }" var="fl" varStatus="status">
					<tr  class="tr" >
						<td class="align-middle">
							<input type="checkbox" id="chk" name="chk" value="${fl.getFl_idx() }" style="width:18px; height:18px;" onclick="chkOne(this);" />
						</td>
						<td>${pi.getNum() - status.index}</td>
						<td>${fl.getFl_ctgr() }</td>
						<td class="text-left"><a href="faqView?flidx=${fl.getFl_idx() }${pi.getArgs() }">${fl.getFl_title() }</a></td>
						<td>${fl.getFl_date()}</td>
		...
		<c:if test="${faqList.size() == 0 }"> 
			<tr height="50"><td colspan="6" align="center">검색결과가 없습니다.</td></tr>
		</c:if>							
		</tbody>
		</table>
		</form>
...
<c:if test="${faqList.size() > 0 }">
    ...
    <c:choose>
		...
		<a class="page-link" href="faqList?cpage=1${pi.getSchargs()}" aria-label="Previous">
            <span aria-hidden="true">«</span>
            <span class="sr-only">Previous</span>
        </a>
        </c:when>
		<c:when test="${ pi.getCpage() > 1 }">
		<a class="page-link" href="faqList?cpage=${pi.getCpage() - 1}${pi.getSchargs()}" aria-label="Previous">
            <span aria-hidden="true">«</span>
            <span class="sr-only">Previous</span>
        </a>
        </c:when>
    ...
    <c:choose>
		...
		<a class="page-link" href="faqList?cpage=${pi.getCpage()}${pi.getSchargs() }" aria-label="Next">
            <span aria-hidden="true">»</span>
            <span class="sr-only">Next</span>
        </a>
        </c:when>
		<c:when test="${pi.getCpage() <  pi.getPcnt()}">
		<a class="page-link" href="faqList?cpage=${pi.getCpage() + 1 }${pi.getSchargs() }" aria-label="Next">
            <span aria-hidden="true">»</span>
            <span class="sr-only">Next</span>
        </a>
        </c:when>

...
public class TicketingCtrl {
	...
	@ResponseBody
	public List<TerminalInfo> getKeywordList(@RequestParam String typeCode, @RequestParam String keyword)
			throws Exception {
	...
	public List<TerminalInfo> getToSpot(@RequestParam String frCode, @RequestParam String typeCode) throws Exception {
		...
		if (typeCode.equals("H"))
			conn.setRequestProperty("x-Gateway-APIKey", H_API_KEY);
		else
	...
	public String ticketingStep02(HttpServletRequest request) throws Exception {
		...
		if (typeCode.equals("H"))
			ri1.setRi_line_id(frCode + toCode);
		...
		if (mode.equals("w")) { // 왕복일경우 오는날 예약정보를 담을 세션 추가생성
			...
			if (typeCode.equals("H"))
				ri2.setRi_line_id(frCode + toCode);
	...
	public String ticketingStep03(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		LocalTime fr_time_local = LocalTime.of(Integer.parseInt(frTime.substring(0, 2)),
				Integer.parseInt(frTime.substring(3)), 00);
		LocalTime to_time_local = LocalTime.of(Integer.parseInt(toTime.substring(0, 2)),
				Integer.parseInt(toTime.substring(3)), 00);
		if (!fr_time_local.isBefore(to_time_local)) {
			LocalDate tmp = LocalDate.of(Integer.parseInt(ri1.getRi_frdate().substring(0, 4)),
					Integer.parseInt(ri1.getRi_frdate().substring(5, 7)),
					Integer.parseInt(ri1.getRi_frdate().substring(8)));
			tmp.plusDays(1);
			ri1.setRi_todate(tmp + " " + toTime + ":00");
		} else
			ri1.setRi_todate(ri1.getRi_frdate() + " " + toTime + ":00");

		if (ri1.getRi_line_type().equals("S"))
			ri1.setRi_line_id(routeId);
	...
	public String ticketingStep05(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		LocalTime to_time_local = LocalTime.of(Integer.parseInt(toTime.substring(0, 2)),
				Integer.parseInt(toTime.substring(3)), 00);
		if (!fr_time_local.isBefore(to_time_local)) {
			LocalDate tmp = LocalDate.of(Integer.parseInt(ri2.getRi_frdate().substring(0, 4)),
					Integer.parseInt(ri2.getRi_frdate().substring(5, 7)),
					Integer.parseInt(ri2.getRi_frdate().substring(8)));
			tmp.plusDays(1);
			ri2.setRi_todate(tmp + " " + toTime + ":00");
		} else
			ri2.setRi_todate(ri2.getRi_frdate() + " " + toTime + ":00");

		if (ri2.getRi_line_type().equals("S"))
			ri2.setRi_line_id(routeId);
	...
	public String payment(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (ri1.getRi_line_type().equals("H"))
			ri1.setRi_line_type("고속");
		else
			ri1.setRi_line_type("시외");
		...
		if (ri1.getMode().equals("p")) { // 편도일 경우
			...
			if (ri2.getRi_line_type().equals("고속"))
				ri2.setRi_line_type("고속");
			else
				ri2.setRi_line_type("시외");
	...
	public String hTicketingResult(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (ri2 != null)
			reservedPayment = ri2.getPayment();
		else if (ri1.getPayment().equals("쿠폰") && totalCnt != 0)
			reservedPayment = ri1.getPayment();
		else
			reservedPayment = ri1.getPayment();

...
<% if ( mi.getMi_pmoney() <= 0) { %>
	           	...
	           	<td colspan="2" class="text-right"><button type="button" class="btn btn-primary" onclick="openModal();" >충전하기</button></td>
	           	</tr>
<% } else { %>	           	
	            <tr>
                  <td class="text-left">
	...
	if (pl.getPd_real_price() > 0) {
	    ...
	    // 이제 formattedNumber는 세 자리마다 쉼표가 찍힌 숫자를 문자열 형태로 저장합니다.
                ...
                <tr>
                  ...
                  <td class="text-left">
                    <p class="mb-0"><%=pl.getRi_line_type() %>버스 예매</p>
                    <p class="mb-0"><%=pl.getRi_fr() %> -> <%=pl.getRi_to() %></p>
                  </td>
                  <td class="text-right"><span class="font-weight-bold mr-1">사용일시</span> <%=pl.getPd_date().substring(0, 16) %></td>
                </tr>
<% } %>
              </tbody>
            </table>
...
<% for (paymoneyInfo mpl : mphList) { 
	...
	if (mpl.getPh_pmoney() > 0) {
	    ...
	    // 이제 formattedNumber는 세 자리마다 쉼표가 찍힌 숫자를 문자열 형태로 저장합니다.
                ...
                <tr>
                  <td class="align-middle"><p class="h2 mb-0">+<%=Ph_pmoney %></p></td>
                  <td class="text-left">
                    <p class="mb-0"><%=mpl.getPh_real_price()/10000 %>만원권 충전</p>
                    <p class="mb-0">10% 추가 적립</p>
                  </td>
                  <td class="text-right"><span class="font-weight-bold mr-1">충전일시</span> <%=mpl.getPh_date().substring(0, 16) %></td>
                </tr>
<% } %>
              </tbody>
            </table>

   ...
   public void setSalesDao(SalesDao salesDao) {
	...
	public List<SalesInfo> getSalesList(String lineType, String terName, String fromDate, String toDate) {
		...
		for (SalesInfo si : salesList) {
			if(si.getCardFee() != 0)	si.setCardRatio(Math.round(((double)si.getCardFee() / (double)si.getTotalFee()  * 100.0) * 10)/10.0);
			if(si.getBankFee() != 0)	si.setBankRatio(Math.round(((double)si.getBankFee() / (double)si.getTotalFee() * 100.0) * 10)/10.0);
			if(si.getEasyFee() != 0)	si.setEasyRatio(Math.round(((double)si.getEasyFee() / (double)si.getTotalFee() * 100.0) * 10)/10.0);
			tmpList.add(si);
		}

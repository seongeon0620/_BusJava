		            ...
		            <div class="card h-100" onclick="imgModal('<%=jo.get("contentid") %>');">
					 ...
					 <% } else out.print(img); %>" class="card-img-top" alt="..." /> 
		              <div class="card-body">
		                ...
		                <h5 class="card-title"><%=jo.get("addr1") %> <%=jo.get("addr2") %></h5>
...
<% if (joItem != null && joItem.size() > 0) { %>
			<li class="page-item">
				<c:choose>
					...
					<a class="page-link" href="javascript:void(0);" aria-label="Previous"></c:when>
					<c:when test="${ pi.getCpage() > 1 }">
			...
			<li class="page-item">
				<c:choose>
					...
					<a class="page-link" href="javascript:void(0);" aria-label="Next"></c:when>
					<c:when test="${pi.getCpage() <  pi.getPcnt()}">
...
<script>
function openModal() {
	$('#ViewModal .modal-content').load("/busjavaf/serviceCtgr");
	$('#ViewModal').modal('show');
}

function imgModal(id) {
	$('#ImgModal .modal-content').load("/busjavaf/detaile?id=" + id);
	$('#ImgModal').modal('show');
}
</script>
